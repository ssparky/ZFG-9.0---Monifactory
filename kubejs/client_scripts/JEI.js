...
JEIEvents.hideItems(event => {
    ...
    // NEW CHANGE - // Simple Compacting Drawers (TEMPORARY UNTIL BUG FIXED)
    ...
    // Hide Artifacts we don't want
    event.hide(['artifacts:everlasting_beef', 'artifacts:eternal_steak', 'moreartifacts:obsidian_skull', 'relics:infinity_ham'])

    // BHC melted hearts
    event.hide("bhc:red_heart_melted")
    event.hide("bhc:yellow_heart_melted")
    event.hide("bhc:green_heart_melted")
    event.hide("bhc:blue_heart_melted")

    // Hide Ice and Fire items we don't want
    event.hide("iceandfire:witherbone")

    // Misc Hiding
    event.hide("supplementaries:flint_block")
    event.hide("supplementaries:flute")
    event.hide("bhc:wither_bone")
    event.hide("bhc:blade_of_vitality")
})
